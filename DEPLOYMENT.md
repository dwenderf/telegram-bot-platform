...
--
--    The content is wrapped in $KENNTNIS_DOC$ ... $KENNTNIS_DOC$ dollar-quoting.
--    *** Paste your content ONLY between the two $KENNTNIS_DOC$ markers. ***
--    Inside those markers, apostrophes and quotes ('  "  ’) need NO escaping —
--    that is the whole point of dollar-quoting. Do NOT alter the markers
--    themselves, and ensure your content does not contain the literal text
--    `$KENNTNIS_DOC# Deployment & Setup Guide
...
The bot answers from `doc_cache`. In v1 you push content **directly** into it (no GitHub sync). Run in the SQL editor as `postgres`.
 ...
 (it won't — that's why this distinctive tag is used).
...
> **Why the `$KENNTNIS_DOC# Deployment & Setup Guide
...
The bot answers from `doc_cache`. In v1 you push content **directly** into it (no GitHub sync). Run in the SQL editor as `postgres`.
 ...
 wrapper (and the #1 SQL-editor footgun):** in the Supabase SQL editor, an **apostrophe in your content** (we're, company's, etc.) will **break the statement** *unless* the content is dollar-quoted — a stray `'` is otherwise read as the end of a SQL string, and everything after it misparses (the editor often greys it out as if commented). Dollar-quoting (`$KENNTNIS_DOC$...$KENNTNIS_DOC# Deployment & Setup Guide
...
) makes everything between the markers a literal, so apostrophes/quotes need **no** escaping. **Two rules:** (1) paste your content **only between** the markers, and (2) **don't disturb the `$KENNTNIS_DOC# Deployment & Setup Guide
...
The bot answers from `doc_cache`. In v1 you push content **directly** into it (no GitHub sync). Run in the SQL editor as `postgres`.
 ...
 markers themselves** — if one gets altered or deleted, dollar-quoting collapses and apostrophes break again. The distinctive tag (`$KENNTNIS_DOC# Deployment & Setup Guide
...
The bot answers from `doc_cache`. In v1 you push content **directly** into it (no GitHub sync). Run in the SQL editor as `postgres`.
 ...
 vs a bare `$`) is chosen so real content can't accidentally contain it.
...
The canonical command list is defined once in [lib/commands.ts](file:///Users/davidwender/Documents/GitHub/telegram-bot-platform/lib/commands.ts).

### Option A: Bulk Sync All Entities (Recommended)
Run the sync script to automatically register/update the commands menu for all bots in the database. This script uses an admin/privileged connection to retrieve and decrypt bot tokens via the RLS tenant context:

```bash
ADMIN_DATABASE_URL="postgres://postgres:your_privileged_postgres_password@db.supabase.co:5432/postgres" \
  npx -y tsx scripts/sync-commands.ts
```

### Option B: Manual Single Bot Autocomplete Setup
For a one-time registration of a single bot during onboarding, you can run a manual `curl` command:
...
> Expect `{"ok":true,"result":true}`. The menu is **per-bot**, so this is a one-time step per entity (each entity has its own bot). Re-run the bulk sync script (Option A) whenever the command set changes. Keep this list in sync with the commands the handler actually implements (currently `/ask`, `/context`, `/whoami`, `/help`; `@mention` is not a slash command so it isn't listed).
>
