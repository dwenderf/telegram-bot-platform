...
- **Progress signals for long-running answers (Pro-tier; pairs with `/draft`).** v1 sends a single `sendChatAction(typing)` before the model call (~5s indicator), which covers a normal `/ask`. For answers that run longer, the coded service (unlike the n8n POC) can keep the user informed, in tiers by duration: **medium (~5–15s)** — a **typing-refresh loop** (re-fire `sendChatAction` every ~4s until the answer is ready) keeps "typing…" continuously visible with zero thread clutter; **long (15s+, e.g. `/draft`)** — a **single editable status message** ("Drafting your presentation…") updated via `editMessageText` and replaced with the result, so there's one updating status line rather than a pile of interim posts (more state to track: hold the `message_id`, handle edit failures). **Important constraint:** these only work *within* the function's allowed execution time — on Vercel **Hobby's ~10s ceiling the function is killed mid-loop**, so progress signals are effectively a **Pro-tier** feature that presupposes a longer duration ceiling. Natural to build alongside `/draft` (the first command long enough to need them) once on Pro. v1 stays with the single typing action.
- **Command personas / behavior config (future direction for write-commands).** Today a command varies only by *context* (per-topic docs) and *capability* (which function it calls). A future enhancement: let each command also carry its own **persona/behavioral rules** — so a command is a bundle of three separable things: **capability** (the action: answer / draft / summarize), **persona+rules** (how it behaves: tone, role, output style — e.g. `/draft` = "you are a presentation drafter, output structured slides, be concise"), and **context scope** (which docs/topics it draws on). This is the right model for "different skills" — **one bot per entity with multiple command-modes**, NOT multiple bots. (Multiple Telegram bots are only warranted if separate *named identities* must coexist in a group — a heavier, rarer need; for varied skills/personas, commands are lighter and keep the conversation unified.) Implementation shape when built: a tenant-scoped `command_config` (command → persona/rules + capability + scope), RLS-protected like all config, naturally per-entity (HYS's `/draft` persona may differ from SymRes's). **Cheap seam to leave now:** keep the §4.1 system-prompt assembly structured so a per-command "rules/persona" block can slot in as one more assembled part, rather than the role preamble being a hardcoded string. Do NOT build the persona system before the write-commands that would use it — it layers on top of `/draft` etc., not before them.
...
- **Group-scoped context resolution.** `manifest_entries.group_id` exists; v1 ignores it (entity-general + topic only). When a second group per entity appears (e.g. HYS Board vs HYS Internal needing different/segregated context), light up group-layer resolution. No migration needed. **The full hierarchy is `entity (1)—<group (N)—<topic (N)`, resolved as a layered MERGE, most-general to most-specific:**
  > - **entity** context (`group_id IS NULL, telegram_thread_id IS NULL`) — shared across all groups.
  > - **group** context (`group_id = G, telegram_thread_id IS NULL`) — that group, all its topics.
  > - **topic** context (`group_id = G, telegram_thread_id = T`) — that group, that topic.
  >
  > A question in (group G, topic T) loads **entity + group + topic** layers, unioned. **`#general` has no topic context of its own** — it's the group's default space, so a question there (null thread) resolves to **entity + group only** (the topic layer is simply empty, which falls out naturally from the merge — no special-casing). **Merge semantics: additive/union by default** (load all matching docs across the three layers); override semantics (a more-specific doc replacing a more-general one) are *not* needed for v1 of this feature and add complexity — default to additive unless a real need for override appears. The `manifest_entries` columns (`group_id`, `telegram_thread_id`, both nullable) already encode all three layers; this is purely a `buildContext` *resolution-logic* change, not a schema change. Pairs naturally with the `/context` command (which shows the resolved layers).
- **Web app / management UI.** The product surface: view/edit docs, manage manifest/topics, onboard entities, abstract GitHub away from non-technical users. Sits on the same capability functions (especially `proposeDocEdit`) as a new front-end. The whole architecture is shaped to make this additive.
  > **Management plane & permissions — see [`MANAGEMENT-PROPOSAL.md`](./MANAGEMENT-PROPOSAL.md).** The web-management subsystem (Supabase Auth web identity, the `/link` claim-code onboarding flow that binds a web user to an entity via Telegram, and the entity-vs-group authorization model) is a *subsystem*, not an increment, so its design lives in its own proposal doc (draft/future — v1 uses manual SQL assignment). It's the management-plane counterpart to the runtime tenant-isolation in `SECURITY-PROPOSAL.md`.
  > **Onboarding-UI constraints & requirements (when built):**
  > - **BotFather cannot be automated.** Telegram gates bot *creation* to real user accounts — there is no API to mint a bot/token. The onboarding flow must therefore be **"bring your own bot"**: guide the tenant through `/newbot` at BotFather (a `t.me/BotFather` deep-link smooths the kickoff) and have them **paste the resulting token**. Everything *after* the token is automatable via the Bot API — validate (`getMe`), store in Vault, `setWebhook`, configure (`setMyCommands`/`setMyDescription`), create entity/group rows. So the one human step is "paste your token"; the platform does the rest. (This is also the cleaner ownership model — the bot lives under the tenant's account, not the platform's, avoiding per-account bot limits and custodial risk.)
  > - **Verify privacy mode is OFF — don't just instruct it (silent-degradation guard).** With Telegram privacy mode ON (the default), the bot only receives commands/mentions/replies, NOT ordinary group chatter — so `message_log` never captures the conversation, and the "resolve references using recent conversation" feature (§4.1, §5) **silently degrades**: the bot answers, but its answers don't reflect the live discussion, with no error to signal why. The onboarding UI must **verify** the state, not trust the instruction: call `getMe` and check `can_read_all_group_messages === true`; **block/blocking-warn** until it's actually off. (Gotcha: privacy setting only applies to groups joined *after* it's changed — if the bot is already in the group, it must be removed and re-added.) See also the privacy-mode health check below.
  > - **Entity-bot connection is an admin-only action.** Anyone can create a bot at BotFather (harmless — it's inert until wired to the platform). The privileged step is *connecting* a bot to an entity (storing its token, pointing its webhook at the entity slug). Restrict this to entity **admins** (the `memberships.role = admin` model) so a member can't hijack an entity's routing by re-onboarding it with a different bot. Schema is **one bot per entity** (entity stores one set of bot credentials).
- **Bot health checks (catch silent misconfiguration).** Several setup steps fail *silently* (the answer is wrong/degraded, nothing errors), which is hard for a non-technical tenant to notice. Build a per-entity health check (the natural home for exposing `checkVaultSecretsHealth`, currently unexposed) that verifies, at onboarding and on demand: (a) Vault secret references resolve; (b) the Telegram webhook is registered; (c) **privacy mode is off** (`getMe` → `can_read_all_group_messages === true` — the recent-conversation feature depends on it, see web-app note above); (d) optionally, that ordinary (non-command) messages are actually arriving in `message_log` for an active group (behavioral confirmation that privacy mode is truly off end-to-end). Turn "a step someone might miss" into "a condition the system verifies."
...
- **Data protection / encryption at rest (pre-public-release).** v1 stores doc content and chat message history as plaintext columns the DB admin (operator) can read. Fine for internal use; for a **public, multi-tenant release** holding other organizations' private content, this needs a deliberate stance. The honest framing distinguishes three things:
  > - **Type A — infrastructure encryption at rest (do this; mostly already provided).** Supabase/Postgres encrypts the database volume at rest at the storage layer. This protects against "physical disk / raw backup leaks." It's transparent (the app reads/writes plaintext; the storage layer handles it) and requires **no architecture change** — verify what's on by default and enable what isn't. This is what "encryption at rest" usually means for SaaS compliance, and it's table stakes for public release.
  > - **Type A.5 — application-layer column encryption with a SERVER-held key (optional, defensible).** Encrypt sensitive columns (doc content, message text) with a key in KMS/Vault (NOT end-user-held). Raises the bar against "DB dump leaks / injection reads raw rows" (ciphertext is useless without the app's key), while the server can still decrypt to function. **Not zero-knowledge** — the operator can still read it.
  > - **Type B — per-user/per-group (zero-knowledge) encryption (REJECTED as a general approach).** Encrypting content with keys only authorized users hold, so the operator *can't* read it, is **fundamentally incompatible with a server-side bot that must read the content to answer questions.** You cannot simultaneously "keep content secret from the server" and "have the server answer questions from it." So Type B is not pursued except possibly for a narrow future zero-knowledge requirement that accepts the functional constraints — likely never.
  >
  > **The irreducible truth:** as long as a server-side bot must read content to answer, *someone operating that server can read it.* No column encryption changes that. **The trust problem is therefore solved structurally, not cryptographically** — via the **self-hosting** option the architecture already enables (AGPL + multi-tenant-but-self-hostable): a privacy-sensitive customer who won't extend operator-trust **runs their own instance** (their Supabase/Vercel/keys), so the operator never holds their data. This yields a clean tier: **hosted** (operator-run, governed by a TOS + Privacy Policy — fine for early release and less-sensitive content) vs **self-hosted** (max privacy, zero operator-trust required — often also the enterprise tier). **TOS + Privacy Policy is the right tool for the hosted-version trust question and is a pre-public-launch LEGAL task (not engineering).** Net: do **Type A** + write **TOS/Privacy Policy** + offer **self-hosting** (already enabled); treat Type A.5 as optional hardening; do not pursue Type B.
